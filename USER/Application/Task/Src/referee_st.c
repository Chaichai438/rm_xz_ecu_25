...
#include "protocol.h"
#define RADAR_MARK_DATA_ID 0x20C
#define SENTRY_INFO_ID 0x20D
#define RADAR_INFO_ID 0x20E
...
void init_referee_struct_data(void)
{
    ...
    //memset(&game_info.supply_projectile_action, 0, sizeof(ext_supply_projectile_action_t));
    memset(&game_info.referee_warning_t, 0, sizeof(ext_referee_warning_t));
	memset(&game_info.dart_remaining_time_t, 0, sizeof(ext_dart_info_t));


    memset(&game_info.game_robot_state, 0, sizeof(ext_robot_state_t));
    memset(&game_info.power_heat_data, 0, sizeof(ext_power_heat_data_t));
    memset(&game_info.game_robot_pos, 0, sizeof(ext_robot_pos_t));
    memset(&game_info.buff_musk, 0, sizeof(ext_buff_t));
    //memset(&game_info.aerial_robot_energy, 0, sizeof(aerial_robot_energy_t));
    memset(&game_info.robot_hurt, 0, sizeof(ext_robot_hurt_t));
    memset(&game_info.shoot_data, 0, sizeof(ext_shoot_data_t));
    ...
    memset(&game_info.round_robot_position, 0, sizeof(ext_ground_robot_position_t));
    memset(&game_info.radar_mark_data, 0, sizeof(radar_mark_data_t));
    memset(&game_info.sentry_info, 0, sizeof(sentry_info_t));    
    memset(&game_info.radar_info, 0, sizeof(radar_info_t));
...
//        break;
		...
		{
            memcpy(&game_info.dart_remaining_time_t, frame + index, sizeof(ext_dart_info_t));								
		}
		break;
        case ROBOT_STATE_CMD_ID:
        {
            memcpy(&game_info.game_robot_state, frame + index, sizeof(ext_robot_state_t));
        }
        break;
        ...
        {
            memcpy(&game_info.game_robot_pos, frame + index, sizeof(ext_robot_pos_t));
        }
        break;
...
//        break;
        case 0x20B:
        {
            memcpy(&game_info.round_robot_position, frame + index, sizeof(ext_ground_robot_position_t));
        }
        break;
        case RADAR_MARK_DATA_ID:
        {
            memcpy(&game_info.radar_mark_data, frame + index, sizeof(radar_mark_data_t));
        }
        break;
        case SENTRY_INFO_ID:
        {
            memcpy(&game_info.sentry_info, frame + index, sizeof(sentry_info_t));
        }
        break;
        case RADAR_INFO_ID:
        {
            memcpy(&game_info.radar_info, frame + index, sizeof(radar_info_t));
        }
		case ROBOT_INTERACTIVE_DATA_ID:
		{
            memcpy(&game_info.custom_robot_data, frame + index, sizeof(custom_robot_data_t));								
		}
        break;
		case ROBOT_CMD_T_ID:
		{
            memcpy(&game_info.map_command, frame + index, sizeof(map_command_t));								
		}
		break;					
		case ROBOT_VERSION_ID:						//键鼠信息
		{
            memcpy(&game_info.remote_control, frame + index, sizeof(remote_control_t));								
		}
		break;
		case MAP_CMD_ID:
		{
            memcpy(&game_info.map_robot_data, frame + index, sizeof(map_robot_data_t));								
		}
		break;
		default:
        {
...
//    *power = game_info.power_heat_data.chassis_power;
    *buffer = 60;//game_info.power_heat_data.buffer_energy;
