...
void UI_task(void const *pvParameters)//绘制ui的函数
{
	/*初始化数据*/
/*hero ui*/
static uint8_t spcell_change_ = 0;
/*hero_end*/
	/**********************************************************************************************************/
		//注：字符型图形只有大写
		// 1920* 1080  左下角为(0,0)
...
//		if(spcell_change_==5)
//		{
//			userUI_draw_constant_power_allowance(UI_NOT_EXTERN, &graph,powerdata[1]);
//		}
		userUI_draw_background();
//		UI_ReFresh(1,graph);
		userUI_draw_constant_power_allowance(UI_IS_EXTERN, &graph,powerdata[1]);
//		userUI_draw_constant_power_allowance(UI_NOT_EXTERN, &graph,powerdata[1]);
		vTaskDelay(20);
