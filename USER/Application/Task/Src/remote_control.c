...
volatile uint8_t  RC_Action_Mouse;						    //鼠标动作

volatile RC_ctrl_t last_rc_ctrl;	
//取正函数
...
void sbus_to_usart6(uint8_t *sbus)
{
    static uint8_t usart_tx_buf[20];
    static uint8_t i =0;
    usart_tx_buf[0] = 0xA6;
...
void RC_Action(void)
{
	unsigned char a;
	//键盘
	for( a = 0; a <= RC_KEY_Z; ++a)
	{
		//按下动作
		if( (last_rc_ctrl.key.v & ( ( uint16_t)0x01 << a)) == 0 && (rc_ctrl.key.v & ( ( uint16_t)0x01 << a)) != 0)
	...
	//鼠标左键
	if( last_rc_ctrl.mouse.press_l == 0 && rc_ctrl.mouse.press_l == 1)			//按下动作
		RC_Action_Mouse |= RC_ACTION_MOUSE_L_PRESS;
	else if( last_rc_ctrl.mouse.press_l == 1 && rc_ctrl.mouse.press_l == 0)		//抬起动作
		RC_Action_Mouse |= RC_ACTION_MOUSE_L_UNPRESS;
	
	
	
	//鼠标右键
	if( last_rc_ctrl.mouse.press_r == 0 && rc_ctrl.mouse.press_r == 1)			//按下动作
		RC_Action_Mouse |= RC_ACTION_MOUSE_R_PRESS;
	else if( last_rc_ctrl.mouse.press_r == 1 && rc_ctrl.mouse.press_r == 0)		//抬起动作
		RC_Action_Mouse |= RC_ACTION_MOUSE_R_UNPRESS;
}

//按键状态变化读取
uint8_t RC_Key_Action_Read( uint8_t rc_key, uint8_t key_action)
{
	//按下
...
//鼠标状态变化获取（按下为1，未按为0）
uint8_t RC_Mouse_Action_Read( uint8_t mouse_action)
{
	if( (RC_Action_Mouse & ( mouse_action)) != 0 )		//读到
	{
...
//按键状态读取（按下为1，未按为0）
uint8_t RC_Key_Read( uint8_t rc_key)
{
	if( (rc_ctrl.key.v & ( 0x01 << rc_key)) != 0)
		return 1;
	return 0;
}
